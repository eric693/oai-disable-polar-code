...
// Function to print an array of int16_t as binary
...
uint32_t semantic_decoder_int16(int16_t *input,
                             uint64_t *out,
                             uint8_t ones_flag,
                             int8_t messageType,
                             uint16_t messageLength,
                             uint8_t aggregation_level )
{
  ...
  int total_bits = aggregation_level * 108;
  printf("\nRX\n");
  printf("rm:");
  ...
  for (int n = 0; n < 63; n++) {
    int n1 = n >> 6;
    int n2 = n & 0x3F; // 等价于 n % 64
    Cprime[n1] |= ((uint64_t)(input[n] < 0 ? 1 : 0) << n2);
  }
