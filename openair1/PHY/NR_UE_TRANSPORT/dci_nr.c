...
// int32_t nr_pdcch_llr_16qam(NR_DL_FRAME_PARMS *frame_parms, int32_t rx_size, int32_t rxdataF_comp[][rx_size],
//                            int16_t *pdcch_llr, uint8_t symbol, uint32_t coreset_nbr_rb) {
//     int16_t *rxF = (int16_t *) &rxdataF_comp[0][(symbol * coreset_nbr_rb * 12)];
//     int32_t i;
//     int16_t *pdcch_llrp;
//     pdcch_llrp = &pdcch_llr[2 * symbol * coreset_nbr_rb * 9 * 4]; // For 16QAM, we need 4 LLRs per symbol
...
//     for (i = 0; i < (coreset_nbr_rb * 9); i++) {
//         int16_t real = rxF[0];
//         int16_t imag = rxF[1];
//         printf("Symbol %d: Real %d, Imag %d\n", i, real, imag);
//         // LLR calculation for 16QAM
//         pdcch_llrp[0] = 7 - 2 * abs(real);
//         pdcch_llrp[1] = 7 - 2 * abs(real - 4);
//         pdcch_llrp[2] = 7 - 2 * abs(imag);
//         pdcch_llrp[3] = 7 - 2 * abs(imag - 4);

//         rxF += 2; // Move to next symbol
//         pdcch_llrp += 4; // Move to next set of 4 LLRs
//     }
...
// void nr_pdcch_channel_compensation_16qam(int32_t rx_size, int32_t rxdataF_ext[][rx_size],
//                                          int32_t dl_ch_estimates_ext[][rx_size], int32_t rxdataF_comp[][rx_size],
//                                          int32_t **rho, NR_DL_FRAME_PARMS *frame_parms, uint8_t symbol,
//                                          uint8_t output_shift, uint32_t coreset_nbr_rb) {
//     uint16_t rb;
//     uint8_t aarx;
...
Bit getBitFromFloatPair(FloatPair pair) {
    if ((pair.first == 0.2500f && pair.second == 0.2500f) ||
        (pair.first == 0.2500f && pair.second == 0.7500f) ||
        (pair.first == 0.2500f && pair.second == 1.2500f) ||
        (pair.first == 0.7500f && pair.second == 0.2500f) ||
        (pair.first == 0.7500f && pair.second == 0.7500f) ||
        (pair.first == 1.2500f && pair.second == 0.2500f) ||
        (pair.first == 1.2500f && pair.second == 0.7500f) ||
        (pair.first == 1.7500f && pair.second == 0.2500f)) {
        return BIT_0;
    } else if ((pair.first == 0.2500f && pair.second == 1.7500f) ||
               (pair.first == 0.7500f && pair.second == 1.2500f) ||
               (pair.first == 0.7500f && pair.second == 1.7500f) ||
               (pair.first == 1.2500f && pair.second == 1.2500f) ||
               (pair.first == 1.2500f && pair.second == 1.7500f) ||
               (pair.first == 1.7500f && pair.second == 0.7500f) ||
               (pair.first == 1.7500f && pair.second == 1.2500f) ||
               (pair.first == 1.7500f && pair.second == 1.7500f)) {
        return BIT_1;
    } else {
        return -1; // Error case
    }
...
void decodeBits(int* bits, int length) {
    for (int i = 0; i < length; i += 4) {
        unsigned char bitSequence = 0;
        for (int j = 0; j < 4 && (i + j) < length; ++j) {
            bitSequence = (bitSequence << 1) | bits[i + j];
        }
                                  ...
                                  fapi_nr_dl_config_dci_dl_pdu_rel15_t *rel15)
{
  ...
  for (int j=0;j<rel15->number_of_candidates;j++) {
    ...
    for (int k = 0; k < rel15->num_dci_options; k++) {
      ...
      uint16_t crc = semantic_decoder_int16(tmp_e,
                                         dci_estimation,
                                         1,
                                         NR_POLAR_DCI_MESSAGE_TYPE, dci_length, L);
      n_rnti = rel15->rnti;

      LOG_D(PHY, "Decoded CRC: %x, Expected RNTI: %x\n", crc, n_rnti);
